...
async function basicAuthentication(req, authHeader) {
  ...
  try {
    if (userNamePasswordArr[0] === process.env.SUPERADMIN_USERNAME) {
      if (userNamePasswordArr[1] === process.env.SUPERADMIN_PASSWORD) {
        req.user = process.env.SUPERADMIN_USERNAME;
        req.role = "";
        req.authType = "basic";
      }
    } else {
      const serviceAccount = userService.getAccount(
        userNamePasswordArr[0]
      );
     await userService.checkLogin(serviceAccount, userNamePasswordArr[1]);
...
export function auth(pathes) {
  return (req, res, next) => {
    const { authentication, authorization } = pathes[req.method][req.url];
    if (!authorization) {
